...
python3 serve_model.py \
    --tracking_uri "http://127.0.0.1:8001" \
    --model_name "Covid-19" \
    --version 1 \
    --port 8010
...
def main():
    parser = argparse.ArgumentParser(
        description="Serve model from MLflow Model Registry"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--version", type=int, help="Specific version to serve (optional)"
    )
    ...
    try:
        ...
        if args.version:
            ...
            version = next(
                (v for v in versions if v.version == str(args.version)), None
            )
            if version is None:
                raise Exception(
                    f"Version {args.version} not found for model '{args.model_name}'"
                )
        else:
